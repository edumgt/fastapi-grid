...
from sqlalchemy import DateTime, ForeignKey, Integer, String, Text, create_engine, func, or_, select
...
allowed_origins = [
    origin.strip()
    for origin in os.getenv("ALLOWED_ORIGINS", "http://127.0.0.1:8001,http://localhost:8001").split(",")
    if origin.strip()
]
app.add_middleware(
    CORSMiddleware,
...
def get_post_or_404(db: Session, post_id: int) -> Post:
    post = db.scalar(select(Post).options(selectinload(Post.owner)).where(Post.id == post_id))
    if not post:
...
def list_posts(
    ...
    rows = db.scalars(
        query.order_by(order_by_map[sort]).offset((page - 1) * size).limit(size)
    ).all()
